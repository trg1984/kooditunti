...
goog.DEPENDENCIES_ENABLED && (goog.included_ = {}, goog.dependencies_ = {
        ...
        for (var b =
                a, c; - 1 != (c = a.indexOf("/./"));) a = a.substr(0, c) + a.substr(c + 2);
        for (; - 1 != (c = a.indexOf("/../"));) {
        ...
        var e = null,
            d = new goog.global.XMLHttpRequest;
        d.onload = function() {
            e = this.responseText
        };
        d.open("get", a, !1);
        d.send();
        e = d.responseText;
        if (null != e) d = goog.wrapModule_(a, e), goog.IS_OLD_IE_ ? (goog.dependencies_.deferred[b] = d, goog.queuedModules_.push(b)) : c(a, d);
        ...
        goog.isDeferredModule_(a) &&
            goog.allDepsAreAvailable_(a) && (a = goog.getPathFromDeps_(a), goog.maybeProcessDeferredPath_(goog.basePath + a))
    ...
    function(a) {
        ...
        try {
            ...
            if (!goog.isString(d) ||
                !d) throw Error('Invalid module name "' + d + '"');
...
goog.Promise = function(a, b) {
    ...
    try {
        ...
        a.call(b, function(a) {
            ...
            if (goog.DEBUG &&
                !(a instanceof goog.Promise.CancellationError)) try {
    ...
    return new goog.Promise(function(b, c) {
        ...
        if (d)
            for (var f = function(a, c) {
                    ...
                    c(a)
                }, h = 0, k; k = a[h]; h++) k.then(goog.partial(f, h), g);
        else b(e)
    })
};
    ...
    return new goog.Promise(function(b, c) {
        ...
        if (d)
            for (var f = function(a) {
                    ...
                    e[a] = b;
                    0 == d && c(e)
                }, h = 0, k; k = a[h]; h++) k.then(f, goog.partial(g, h));
        else b(void 0)
    })
};
...
goog.Promise.prototype.executeCallbacks_ = function() {
    for (; this.callbackEntries_ && this.callbackEntries_.length;) {
        ...
        for (var b = 0; b < a.length; b++) goog.Promise.LONG_STACK_TRACES && this.currentStep_++, this.executeCallback_(a[b], this.state_, this.result_)
    }
    this.executing_ = !1
};
goog.Promise.prototype.executeCallback_ = function(a, b, c) {
    if (b == goog.Promise.State_.FULFILLED) a.onFulfilled(c);
    else a.child && this.removeUnhandledRejection_(), a.onRejected(c)
};
...
goog.a11y.aria.toggleState = function(a, b) {
    ...
    goog.string.isEmptySafe(c) || "true" == c || "false" == c ? goog.a11y.aria.setState(a, b, "true" == c ? "false" : "true") : goog.a11y.aria.removeState(a, b)
};
...
goog.html.SafeHtml.create = function(a, b, c) {
    ...
    return goog.html.SafeHtml.create_(a, null, b, c)
};
goog.html.SafeHtml.createEmbed = function(a, b, c) {
    if (goog.string.isEmpty(goog.string.Const.unwrap(b))) throw Error('Must provide non-empty "type"');
    for (var d in c) {
        var e = d.toLowerCase();
        if ("src" == e || "type" == e) throw Error('Cannot override "src" nor "type" attributes, got "' + d + '" with value "' + c[d] + '"');
    }
    a = goog.string.htmlEscape(goog.html.TrustedResourceUrl.unwrap(a));
    b = goog.string.htmlEscape(goog.string.Const.unwrap(b));
    return goog.html.SafeHtml.create_("embed", 'src="' + a + '" type="' + b + '"', c)
};
goog.html.SafeHtml.create_ = function(a, b, c, d) {
    ...
    if (c)
        for (var g in c) {
            if (!goog.html.SafeHtml.VALID_NAMES_IN_TAG_.test(g)) throw Error('Invalid attribute name "' + g + '".');
            b = c[g];
            goog.isDefAndNotNull(b) && (f += " " + goog.html.SafeHtml.getAttrNameAndValue_(a, g, b))
        }
    goog.isDef(d) ? goog.isArray(d) || (d = [d]) : d = [];
    goog.dom.tags.isVoidTag(a.toLowerCase()) ? (goog.asserts.assert(!d.length, "Void tag <" + a + "> does not allow content."), f += ">") : (e = goog.html.SafeHtml.concat(d), f += ">" + goog.html.SafeHtml.unwrap(e) +
        "</" + a + ">", e = e.getDirection());
    (a = c && c.dir) && (e = /^(ltr|rtl|auto)$/i.test(a) ? goog.i18n.bidi.Dir.NEUTRAL : null);
...
goog.ui.tree.BaseNode.prototype.createDom = function() {
    var a = this.getDomHelper().htmlToDocumentFragment(goog.html.SafeHtml.unwrap(this.toSafeHtml()));
    this.setElementInternal(a)
};
...
Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
    ...
    for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = this.block_.inputsInline && !this.block_.isCollapsed(), p = 0, m; m = b[p]; p++)
        if (m.isVisible()) {
            ...
            for (var n = !1, r = 0, s; s = m.fieldRow[r]; r++) {
                0 != r && (m.fieldWidth += Blockly.BlockSvg.SEP_SPACE_X);
                var t = s.getSize();
                s.renderWidth = t.width;
                s.renderSep = n && s.EDITABLE ? Blockly.BlockSvg.SEP_SPACE_X : 0;
                m.fieldWidth += s.renderWidth + s.renderSep;
                q.height = Math.max(q.height, t.height);
                n = s.EDITABLE
            }
...
Blockly.Trashcan.prototype.animateLid_ = function() {
    ...
    if (0 < this.lidOpen_ || 1 > this.lidOpen_) this.lidTask_ = goog.Timer.callOnce(this.animateLid_, 20, this)
};
...
Blockly.Workspace.prototype.recordDeleteAreas = function() {
    ...
    this.deleteAreaToolbox_ = this.flyout_ ? this.flyout_.getRect() : null
};
Blockly.Workspace.prototype.isDeleteArea = function(a) {
...
Blockly.Connection.prototype.connect = function(a) {
    ...
    var d;
    this.isSuperior() ? (c = this.sourceBlock_, d = a.sourceBlock_) : (c = a.sourceBlock_,
        d = this.sourceBlock_);
...
Blockly.Mutator.prototype.createEditor_ = function() {
    ...
    this.workspace_.flyout_.autoClose = !1;
...
Blockly.Block.prototype.onMouseUp_ = function(a) {
    ...
    Blockly.doCommand(function() {
        Blockly.terminateDrag_();
        if (Blockly.selected && Blockly.highlightedConnection_) Blockly.localConnection_.connect(Blockly.highlightedConnection_), b.svg_ && (Blockly.localConnection_.isSuperior() ? Blockly.highlightedConnection_ : Blockly.localConnection_).sourceBlock_.svg_.connectionUiEffect(), b.workspace.trashcan && b.workspace.trashcan.close();
        else if (b.workspace.isDeleteArea(a)) {
            var c = b.workspace.trashcan;
            c && goog.Timer.callOnce(c.close,
                100, c);
...
Blockly.Block.prototype.onMouseMove_ = function(a) {
    ...
    Blockly.doCommand(function() {
        if (!("mousemove" == a.type && 1 >= a.clientX && 0 == a.clientY && 0 == a.button)) {
            ...
            1 == Blockly.Block.dragMode_ && Math.sqrt(Math.pow(c, 2) + Math.pow(d, 2)) > Blockly.DRAG_RADIUS && (Blockly.Block.dragMode_ = 2, b.setParent(null), b.setDragging_(!0), b.workspace.recordDeleteAreas());
            if (2 == Blockly.Block.dragMode_) {
                var e = b.startDragX + c,
                    f = b.startDragY + d;
                b.svg_.getRootElement().setAttribute("transform", "translate(" + e + ", " + f + ")");
...
Blockly.Toolbox.prototype.init = function() {
    ...
    var a = new Blockly.Toolbox.TreeControl(this, this.CONFIG_);
    this.tree_ = a;
    a.setShowRootNode(!1);
    a.setShowLines(!1);
        ...
        for (var e = 0, f; f = c.childNodes[e]; e++)
            if (f.tagName) {
                ...
                if ("CATEGORY" == g) {
                    ...
                    h ? g.blocks = h : a(f, g)
                } else "HR" == g ? d.add(new Blockly.Toolbox.TreeSeparator) : "BLOCK" == g && d.blocks.push(f)
            }
    }
    var b = this.tree_;
    b.removeChildren();
...
Blockly.Toolbox.TreeControl.prototype.setSelectedItem = function(a) {
    this.selectedItem_ != a && (goog.ui.tree.TreeControl.prototype.setSelectedItem.call(this, a), a && a.blocks && a.blocks.length ? this.toolbox_.flyout_.show(a.blocks) : this.toolbox_.flyout_.hide())
};
...
Blockly.Css.inject = function() {
    ...
    a = goog.cssom.getAllCssStyleSheets();
    Blockly.Css.styleSheet_ = a[a.length - 1];
...
Blockly.Css.setCursor = function(a) {
    if (!Blockly.readOnly && Blockly.Css.currentCursor_ != a) {
        ...
        Blockly.svg && (Blockly.svg.style.cursor = a == Blockly.Css.Cursor.OPEN ? "" : b)
    }
};
...
Blockly.createDom_ = function(a) {
    ...
    Blockly.readOnly ||
        (Blockly.hasCategories ? Blockly.mainWorkspace.toolbox_ = new Blockly.Toolbox(b, a) : (Blockly.mainWorkspace.flyout_ = new Blockly.Flyout, c = Blockly.mainWorkspace.flyout_, d = c.createDom(), c.autoClose = !1, goog.dom.insertSiblingBefore(d, Blockly.mainWorkspace.svgGroup_), Blockly.addChangeListener(function() {
            if (0 == Blockly.Block.dragMode_) {
                var a = Blockly.mainWorkspace.getMetrics();
                if (0 > a.contentTop || a.contentTop + a.contentHeight > a.viewHeight + a.viewTop || a.contentLeft < (Blockly.RTL ? a.viewLeft : 0) || a.contentLeft + a.contentWidth >
                    (Blockly.RTL ? a.viewWidth : a.viewWidth + a.viewLeft))
...
Blockly.init_ = function() {
    ...
    if (Blockly.languageTree)
        if (Blockly.mainWorkspace.toolbox_) Blockly.mainWorkspace.toolbox_.init();
    ...
    if (Blockly.hasSounds) {
        Blockly.loadAudio_([Blockly.pathToMedia + "click.mp3", Blockly.pathToMedia + "click.wav", Blockly.pathToMedia + "click.ogg"], "click");
        Blockly.loadAudio_([Blockly.pathToMedia +
            "delete.mp3", Blockly.pathToMedia + "delete.ogg", Blockly.pathToMedia + "delete.wav"
        ], "delete");
        var b = [],
            a = function() {
